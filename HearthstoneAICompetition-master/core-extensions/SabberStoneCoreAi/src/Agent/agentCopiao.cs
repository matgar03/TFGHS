...
{
	class ScoreHeimbrodth : Score.Score
	{
	...
	class agentCopiao : AbstractAgent
	{
		...
		public override PlayerTask GetMove(SabberStoneCoreAi.POGame.POGame game)
		{
			...
			KeyValuePair<PlayerTask, int> score(KeyValuePair<PlayerTask, POGame.POGame> state, int player_id, int max_depth = 3)
			{
		...
		private static int Score(POGame.POGame state, int playerId)
		{
			...
			return new ScoreHeimbrodth { Controller = p }.Rate();
		}
